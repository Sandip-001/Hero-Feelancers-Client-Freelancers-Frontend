...
export default function ClientDashboardPage() {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-8 bg-slate-50 min-h-screen font-sans text-slate-900">
        ...
        <div className="xl:col-span-1 flex flex-col h-full sticky top-8">
          ...
          <div className="flex-1 bg-white rounded-2xl border border-slate-200 shadow-sm flex flex-col overflow-hidden mt-4 min-h-[600px]">
              ...
              <div className="flex items-center gap-3">
                <div className="relative">
                  <img
                    ...
                    className="w-10 h-10 rounded-full bg-indigo-50 border border-slate-100"
                  />
            ...
            <div className="flex-1 p-4 overflow-y-auto space-y-6 bg-slate-50/50 custom-scrollbar">
              <Bubble
                text="Hi! Just updated the homepage layout based on your feedback."
                time="10:30 AM"
              />
              <Bubble
                text="That was quick! Can we adjust the hero section slightly?"
                me
                time="10:32 AM"
              />
              <Bubble
                text="Sure, making those tweaks now. Will share a revision shortly."
                time="10:35 AM"
              />
              ...
              <Bubble
                text="Here is the updated design file."
                file="Homepage_v2.fig"
                time="2:15 PM"
              />
            </div>
      ...
      <ClientDevelopmentPopup   name="Client Dashboard" />
    </div>
  );
}
...
function SectionHeader({ title }: { title: string }) {
  return <h3 className="text-xl font-bold text-slate-800">{title}</h3>;
}
  ...
  return (
    <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-sm hover:shadow-md transition-shadow duration-300">
      <div className="flex justify-between items-start mb-4">
        <div
          className={`p-3 rounded-xl ${colorStyles[color]} ring-1 ring-inset ring-black/5`}
        >
          <Icon size={22} />
        </div>
        {warning && (
      ...
      <div>
        ...
        <div className="flex items-center gap-1 mt-2">
          <span
            className={`text-xs font-medium ${trendUp === true ? "text-emerald-600" : trendUp === false && warning !== true ? "text-rose-500" : "text-slate-400"}`}
          >
            {trend}
          </span>
        </div>
    ...
    <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:border-indigo-200 transition-colors">
      <div>
        ...
        <div className="text-sm text-slate-500 mt-1 flex flex-wrap gap-x-2">
          <span>
            Freelancer:{" "}
            <span className="text-slate-700 font-medium">{freelancer}</span>
          </span>
  ...
  return (
    <div className={`flex flex-col ${me ? "items-end" : "items-start"}`}>
      ...
      >
        {file && (
          <div
            className={`flex items-center gap-3 p-2.5 rounded-lg mb-2 ${me ? "bg-indigo-500/50" : "bg-slate-100"}`}
          >
