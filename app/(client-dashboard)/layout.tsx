...
const INITIAL_NOTIFICATIONS = [
  {
    ...
    message: "Alice Dev submitted a proposal...",
    time: "2 min ago",
    type: "proposal",
    read: false,
  },
  {
    ...
    message: "Backend API integration...",
    time: "1 hour ago",
    type: "milestone",
    read: false,
  },
            ...
            <div className="flex items-center gap-2 sm:gap-4">
              <div className="hidden sm:flex items-center bg-slate-100 rounded-full px-4 py-2 border border-slate-200 focus-within:ring-2 focus-within:ring-indigo-500/20 focus-within:border-indigo-500 transition-all">
                ...
                <input
                  ...
                  className="bg-transparent border-none focus:ring-0 text-sm ml-2 w-32 md:w-48 placeholder:text-slate-400"
                />
              </div>
                      ...
                      <div className="flex items-center gap-2">
                        {notifications.length > 0 && (
                          <button
                            ...
                            className="text-xs text-indigo-600 hover:text-indigo-700 font-semibold px-2 py-1 hover:bg-indigo-50 rounded-md transition-colors"
                          >
                            Clear all
                          </button>
                        )}
                      ...
                      {notifications.length === 0 ? (
                        ...
                        <ul className="divide-y divide-slate-50">
                          {notifications.map((item) => (
                            <li
                              key={item.id}
                              className="group hover:bg-slate-50/80 transition-colors cursor-pointer px-4 py-4 flex gap-4"
                            >
                              <div
                                className={`mt-1 flex-shrink-0 w-10 h-10 rounded-xl flex items-center justify-center ${item.type === "proposal" ? "bg-indigo-50 text-indigo-600" : item.type === "payment" ? "bg-emerald-50 text-emerald-600" : "bg-blue-50 text-blue-600"}`}
                              >
