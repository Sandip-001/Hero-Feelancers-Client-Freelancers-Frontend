...
export default function DashboardPage() {
  ...
  const {
    ...
    isLoading,
    error,
  } = useGetMyProposalsQuery({ freelancerId }, { skip: !freelancerId });
  ...
  const { activeProjects, stats } = useMemo(() => {
    ...
    const active = list.filter(
      (p: any) => p.status === "Awarded" || p.status === "Accepted",
    );
      ...
      stats: {
        ...
        earnings: [...active, ...completed].reduce((acc: number, curr: any) => {
          const val = parseFloat(
            (curr.proposedAmount || curr.bidAmount || "0")
              .toString()
              .replace(/[^0-9.]/g, "") || "0",
          );
          return acc + val;
  ...
  return (
    <div className="flex bg-gray-50 min-h-screen p-3 xl:p-6 lg:pl-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {isLoading ? (
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <div className="flex items-center justify-between pt-2">
                      ...
                      <Link href="/proposals">
                        <Button
                          size="sm"
                          className="bg-[#14A9F9] hover:bg-blue-700"
                        >
                          Details
                        </Button>
                      </Link>
              ...
              <div className="col-span-3 py-12 text-center bg-white rounded-xl border border-dashed border-gray-300 text-gray-500">
                ...
                <Link
                  href="/projects"
                  className="text-indigo-600 text-sm font-bold hover:underline mt-2 inline-block"
                >
                  Find Work
                </Link>
              </div>
      ...
      <DevelopmentPopup name={"Dashboard"} />
    </div>
  );
}
