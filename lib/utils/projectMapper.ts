...
export const mapApiJobToUI = (apiJob: any): Project => {
  ...
  return {
    ...
    budgetValue: apiJob.projectValue ? Number(apiJob.projectValue) : 0,

    type:
      (apiJob.priceType || "").toLowerCase() === "hourly" ? "Hourly" : "Fixed",

    experienceLevel: apiJob.experienceLevel || "Intermediate",
    // 🔥 FIX 1: Convert to timestamp number
    postedTime: apiJob.createdAt,

    // 🔥 FIX 2: Add paymentVerified
    paymentVerified: clientInfo.isEmailVerified || false,
    ...
    status:
      apiJob.status === "Open" || apiJob.status === "Active"
        ? "Pending"
        : apiJob.status,
    ...
    client: {
      ...
      location: clientInfo.countryName || "Remote",
      rating: clientInfo.rating || 0,
      totalSpent: clientInfo.totalSpent || "₹0",
      memberSince: clientInfo.createdAt
        ? new Date(clientInfo.createdAt).getFullYear().toString()
        : "2024",
      verified: clientInfo.isEmailVerified || false,
    },

    // Map existing proposal details if present (for Applied/Awarded tabs)
    myBidAmount: apiJob.myProposal?.bidAmount
      ? `₹${apiJob.myProposal.bidAmount}`
      : undefined,
